...
class FileSorterApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(
            opts, text="Recursive (include subfolders)", variable=self.recursive_var
        ).pack(side="left")

        ttk.Checkbutton(
            opts, text="Include hidden files", variable=self.hidden_var
        ...
        self.open_btn = ttk.Button(btns, text="Open Folder", command=self._on_open_folder)
        self.open_btn.pack(side="left", padx=(10, 0))

        # Progress (directly under buttons)
        prog = ttk.Frame(root)
        ...
        self.progress_label = ttk.Label(prog, text="", width=40, anchor="w")
    ...
    def _progress_set(self, current: int, total: int, item_text: str) -> None:
        self.progress["maximum"] = max(total, 1)
        self.progress["value"] = current
        # keep text short-ish
        self.progress_label.config(text=f"{current}/{total}  {item_text[:60]}")
        self.update_idletasks()  # force redraw (helps when operations are fast)
    ...
    def _on_browse(self) -> None:
        ...
        if path:
            ...
            self.undo_btn.config(state="normal" if self._compute_log_path(Path(path)).exists() else "disabled")

    def _on_open_folder(self) -> None:
        root = Path(self.folder_var.get()).expanduser()
        if root.exists():
            try:
                os.startfile(root)  # Windows
    ...
    def _on_preview(self) -> None:
        ...
        def work():
            try:
                self.after(0, self._progress_reset)
    ...
    def _show_preview(self, root: Path, plan, log_path: Path) -> None:
        ...
        self._clear_table()
        for item in plan:
            self.tree.insert("", "end", values=(item.category, str(item.src), str(item.dst)))

        if len(plan) == 0:
            self.status_var.set("No moves planned. (Already sorted, empty, or excluded by options.)")
            self.sort_btn.config(state="disabled")
        else:
    ...
    def _on_sort(self) -> None:
        ...
        def work():
            try:
                ...
                def on_progress(i, total, item):
                    # background thread -> schedule UI update on Tk thread
                    self.after(0, lambda: self._progress_set(i, total, f"{item.category}: {item.src.name}"))
    ...
    def _after_sort(self, root: Path, log_path: Path, moved_count: int) -> None:
        # Leave bar at 100% so you can SEE it worked
        self._progress_set(moved_count, moved_count, "Done")
        self.status_var.set(f"Moved {moved_count} files. Log saved: {log_path}")
        self.undo_btn.config(state="normal")

        # Clear preview (folder has changed)
        self._plan = []
        self.sort_btn.config(state="disabled")
        self._clear_table()
    ...
    def _after_undo(self, log_path: Path, restored: int) -> None:
        # No per-file undo progress yet; just show completion.
        self._progress_set(restored, restored, "Undo done")
